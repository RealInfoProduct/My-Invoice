...
@Component({
  selector: 'app-credit-note-pdfview',
  templateUrl: './pdfview.component.html',
  styleUrls: ['./pdfview.component.scss']
})
export class CreditNotePdfViewComponent implements OnInit {
  blobUrl: any;
  creditNoteData: any;
  firmList: any = [];
  partyList: any = [];
  ...
  ngOnInit(): void {
    ...
    this.firebaseService.getAllFirm().subscribe((res: any) => {
      if (res) {
        ...
        if (this.loaderService.getCreditNoteData()) {
          this.creditNoteData = this.loaderService.getCreditNoteData();
          if (!this.creditNoteData.firmName && this.creditNoteData.firmId) {
            this.creditNoteData.firmName = this.firmList.find((f: any) => f.id === this.creditNoteData.firmId);
          }
          this.generatePDF(this.creditNoteData);
        } else {
  ...
  back() {
    this.loaderService.setCreditNoteData(this.creditNoteData);
    this.router.navigate(['/master/addcreditnote']);
  }

  generatePDF(creditNoteData: any) {
    ...
    const fieldsRightValues = [
      `${creditNoteData.creditNoteNumber || 'Draft'}`,
  ...
  submitCreditNote() {
    ...
    const firmId = this.creditNoteData.firmId;
    const accountYear = this.creditNoteData.accountYear || localStorage.getItem('accountYear') || '';
    this.firebaseService.getLatestCreditNoteNumber(userId, firmId, accountYear).then(nextCNNumber => {
      this.creditNoteData.creditNoteNumber = nextCNNumber;

      this.firebaseService.addCreditNote(this.creditNoteData).then((res) => {
        if (res) {
          // Generate PDF download
          this.pdfgenService.generateCreditNotePDFDownload(this.creditNoteData);
          this.openConfigSnackBar('Record created successfully');
          this.loaderService.setCreditNoteData(null);
          this.router.navigate(['/master/creditNote']);
        }
      }, (error) => {
        this.openConfigSnackBar(error.message || 'Error saving credit note');
        this.loaderService.setLoader(false);
      });
    }).catch(err => {
      this.openConfigSnackBar('Error generating credit note number');
      this.loaderService.setLoader(false);
    });
  }
